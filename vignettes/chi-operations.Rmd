...
CHI numbers beginning with certain dates of birth, particularly those on 1st January, are running into the issue of there being no more CHI numbers that can be generated that conform to the Modulo 11 check digit, as all combinations have already been assigned to individuals. In this circumstance, CHI numbers that conform to the Modulo 10 check digit will be allowed to be generated instead after the implementation date in February 2026. By default, `chi_check()` will output as 'Valid CHI' if the CHI numbers passes either a Mod-11 or Mod-10 check. However it will not apply complicated rules such as using a Mod-10/Mod-11 check on only CHI numbers relating to dates of birth starting on 1st January and using the Mod-11 check otherwise. If you want to apply a check following these rules, `if_else()` or `case_when()` can be used.
