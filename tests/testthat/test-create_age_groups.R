test_that("Return correct age groups", {
  expect_identical(
    create_age_groups(c(4, 51, 21, 89), 0, 80, 10, as_factor = FALSE),
    c("0-9", "50-59", "20-29", "80+")
  )

  expect_identical(
    create_age_groups(c(8, 94, 44, 55, 14), 0, 90, 5, as_factor = TRUE),
    factor(
  ...
  expect_identical(
    create_age_groups(c(81, 86, 33, 11), 4, 84, 3, as_factor = FALSE),
    c("79-81", "82+", "31-33", "10-12")
  )

  expect_identical(
    create_age_groups(c(0, 99, 1000, 5, 5), 5, 90, 5, as_factor = FALSE),
    c(NA, "90+", "90+", "5-9", "5-9")
  )
})
