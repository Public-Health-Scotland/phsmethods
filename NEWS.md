...
work but issue a warning as they will eventually be removed in a future version.
...
- Three functions that were renamed and deprecated in [phsmethods 0.2.1 (2022-02-11)](#phsmethods-021-2022-02-11) have now been removed: `postcode()` is now `format_postcode()`; `age_group()` is now `create_age_groups()` and `fin_year()` is now `extract_fin_year()`.
